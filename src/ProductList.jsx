...
import { addItem } from "./CartSlice"; // path senin dizin yapına göre
...
function ProductList({ onHomeClick }) {
  ...
  const handleAddToCart = (product) => {
    dispatch(addItem(product)); // Redux’a gönder
    setAddedToCart((prevState) => ({
      ...prevState,
      [product.name]: true, // Ürünü “added” olarak işaretle
    }));
  };
      ...
      {!showCart ? (
        <div className="product-grid">
          {plantsArray.map(
            (
              category,
              index // Loop through each category
            ) => (
              <div key={index}>
                <h1>
